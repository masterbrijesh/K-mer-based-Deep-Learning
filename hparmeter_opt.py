...
def train_mnist(config):
    ...
    # Load model
    net = AlexNet(config["l1"], config["l2"]).to(device)
    ...
    # tune.report(mean_accuracy=accuracy)  # 添加的代码
...
config = {
    ...
    "l1": tune.qlograndint(64, 256, 8),
    "l2": tune.qlograndint(32, 256, 8),
    "lr": tune.quniform(0.01, 0.0005,0.00025),
...
net = AlexNet(best_config["l1"], best_config['l2']).to(device) # Set the model and move to GPU.
